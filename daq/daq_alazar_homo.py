...
def get_alazar_parameters(daq_params=None):
    ...
    alazar_params.records_per_buffer = min(
        1 * 3072, alazar_params.num_total_records
    )  # #3072 #8192 For 8192 I changed 2*3072 to 1*3072
...
def configure_board(alazar_params, board):
    ...
    # TODO: Select channel A input parameters as required. allowed 40 50 80 100 200 400 500 800 mV 1 2 4 V
    board.inputControlEx(
        ats.CHANNEL_A, ats.DC_COUPLING, ats.INPUT_RANGE_PM_200_MV, ats.IMPEDANCE_50_OHM
    )
    ...
    board.inputControlEx(
        ats.CHANNEL_B, ats.DC_COUPLING, ats.INPUT_RANGE_PM_200_MV, ats.IMPEDANCE_50_OHM
    )
...
def acquire_data_het(daq_params, alazar_params, board, ssm_if=0.02, verbose=True):
    ...
    try:
        ...
        srs_dg535.set_trigger_level(
            "low"
        )  # dg535_control.set_state(0) #serra added and commented out
...
def acquire_data_het_2q(
    ...
    #            pbar.update(1)
        ...
        srs_dg535.set_trigger_level(
            "low"
        )  # dg535_control.set_state(0) #alexandria recently commented out 11/01/2023
    # print("\ndaq_alazar Troubleshoot stuck at this step 16")
    # Compute the total transfer time, and display performance information.
    # print("H")
    if verbose:
...
def acquire_data(daq_params, alazar_params, board):
    ...
    index_ch = [None] * 2
    index_ch[0] = np.arange(0, post_trigger_samples * records_per_buffer)  # channel A
    index_ch[1] = post_trigger_samples * records_per_buffer + np.arange(
        0, post_trigger_samples * records_per_buffer
    )  # channel B
    ...
    try:
        board.startCapture()  # Start the acquisition
        print("Capturing %d buffers. Press <enter> to abort" % buffers_per_acquisition)
        buffersCompleted = 0
        bytesTransferred = 0
        while buffersCompleted < buffers_per_acquisition and not ats.enter_pressed():
            # Wait for the buffer at the head of the list of available
            # buffers to be filled by the board.
            buffer = buffers[buffersCompleted % len(buffers)]
            board.waitAsyncBufferComplete(
                buffer.addr, timeout_ms=500000
            )  # was 5000 earlier
            buffersCompleted += 1
            bytesTransferred += buffer.size_bytes
    ...
    if transferTime_sec > 0:
        ...
        recordsPerSec = records_per_buffer * buffersCompleted / transferTime_sec
    print(
        "Captured %d buffers (%f buffers per sec)" % (buffersCompleted, buffersPerSec)
    )
    print(
        "Captured %d records (%f records per sec)"
        % (records_per_buffer * buffersCompleted, recordsPerSec)
...
def acquire_data_raw(daq_params, alazar_params, board):
    ...
    rec_all_raw_ave = np.zeros(
        (buffers_per_acquisition, 2, records_per_buffer, post_trigger_samples)
    )
    rec_avg_all = [None] * 2
    rec_readout = [[]] * 2
