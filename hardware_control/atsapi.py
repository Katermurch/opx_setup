...
"""
Board input ranges (amplitudes) identifiers. PM stands for plus/minus.
...
"""Parameters that apply to some modes of the auxiliary input/output port."""
TRIGGER_SLOPE_POSITIVE = 1
TRIGGER_SLOPE_NEGATIVE = 2
...
def returnCodeCheck(result, func, arguments):
    ...
    if result != 512:
        raise Exception(
            ...
            % (func.__name__, str(arguments), str(ats.AlazarErrorToText(result)))
        )
...
ats.AlazarDSPGenerateWindowFunction.argtypes = [
    c_uint32,
    POINTER(c_float),
    c_uint32,
    c_uint32,
...
def dspGenerateWindowFunction(windowType, windowLength_samples, paddingLength_samples):
    """Fills an array with a generated window function and pads it with
    zeros."""
    window = np.zeros(windowLength_samples + paddingLength_samples, dtype=np.float32)
    ats.AlazarDSPGenerateWindowFunction(
        windowType,
...
ats.AlazarExtractFFTNPTFooters.argtypes = [
    c_void_p,
    c_uint32,
    c_uint32,
    POINTER(NPTFooter),
...
def extractFFTNPTFooters(
    buffer, recordSize_bytes, bufferSize_bytes, footersArray, numFootersToExtract
):
    ats.AlazarExtractFFTNPTFooters(
        buffer, recordSize_bytes, bufferSize_bytes, footersArray, numFootersToExtract
    )


ats.AlazarExtractTimeDomainNPTFooters.restype = c_uint32
ats.AlazarExtractTimeDomainNPTFooters.argtypes = [
    c_void_p,
    c_uint32,
    c_uint32,
    POINTER(NPTFooter),
...
def extractTimeDomainNPTFooters(
    buffer, recordSize_bytes, bufferSize_bytes, footersArray, numFootersToExtract
):
    ats.AlazarExtractTimeDomainNPTFooters(
        buffer, recordSize_bytes, bufferSize_bytes, footersArray, numFootersToExtract
    )
...
ats.AlazarGetDriverVersion.argtypes = [
    POINTER(c_byte),
    POINTER(c_byte),
    POINTER(c_byte),
]
...
class DspModule:
    ...
    def dspGetInfo(self):
        ...
        ats.AlazarDSPGetInfo(
            self.handle, byref(id), byref(major), byref(minor), byref(maxLength), 0, 0
        )
        return (id.value, major.value, minor.value, maxLength.value)

    ats.AlazarFFTGetMaxTriggerRepeatRate.restype = c_uint32
    ats.AlazarFFTGetMaxTriggerRepeatRate.argtypes = [
        handle_t,
        c_uint32,
        POINTER(c_double),
    ]
    ...
    ats.AlazarFFTSetWindowFunction.argtypes = [
        handle_t,
        c_uint32,
        POINTER(c_float),
        POINTER(c_float),
    ...
    def fftSetWindowFunction(self, samplesPerRecord, realWindowArray, imagWindowArray):
        ats.AlazarFFTSetWindowFunction(
            self.handle, samplesPerRecord, realWindowArray, imagWindowArray
        )

    ats.AlazarFFTSetup.restype = c_uint32
    ats.AlazarFFTSetup.argtypes = [
        handle_t,
        c_uint16,
        c_uint32,
        c_uint32,
    ...
    ats.AlazarFFTVerificationMode.argtypes = [
        handle_t,
        c_uint32,
        POINTER(c_int16),
        POINTER(c_int16),
    ...
    def fftSetScalingAndSlicing(self, u52_slice_pos, loge_ampl_mult):
        ...
        ats.AlazarFFTSetScalingAndSlicing(self.handle, u52_slice_pos, loge_ampl_mult)
    ...
    def dspOutputSnoopConfig(self, wraparound, oneShot, freeze):
        ats.AlazarDSPOutputSnoopConfig(
            self.handle, 1 if wraparound else 0, 1 if oneShot else 0, 1 if freeze else 0
        )
    ...
    def dspOutputSnoopStatus(self):
        ...
        ats.AlazarDSPOutputSnoopStatus(
            self.handle,
            byref(outFrozen),
            byref(outMaxRecSize_u32),
            byref(outLastRecSize_u32),
        )
        return (True if outFrozen else False, outMaxRecSize_u32, outLastRecSize_u32)

    restype = c_uint32
    argtypes = [handle_t, c_uint32, c_void_p, c_uint32, c_void_p]
    errcheck = returnCodeCheck

    def dspOutputSnoopRead(self, bytesPerSample, outputArray, outputArraySize_samples):
        writtenSamples = c_uint32(0)
        ats.AlazarDSPOutputSnoopRead(
            self.handle,
            bytesPerSample,
            outputArray,
            outputArraySize_samples,
            byref(writtenSamples),
        )
        return writtenSamples.value

    ats.AlazarDISSetup.restype = c_uint32
    ats.AlazarDISSetup.argtypes = [
        handle_t,
        c_uint32,
        c_uint16,
        POINTER(c_float),
    ...
    def disSetup(
        self, options, inputChannelMask, gainArray, offsetArray, saturationArray
    ):
        ...
        ats.AlazarDISSetup(
            ...
            inputChannelMask,
            (
                disGain.ctypes.data_as(POINTER(c_float))
                if len(disGain)
                else POINTER(c_float)()
            ),
            (
                disOffset.ctypes.data_as(POINTER(c_int32))
                if len(disOffset)
                else POINTER(c_int32)()
            ),
            (
                disSaturation.ctypes.data_as(POINTER(c_int32))
                if len(disSaturation)
                else POINTER(c_int32)()
            ),
        )
    ...
    ats.AlazarFFTBackgroundSubtractionGetRecordS16.argtypes = [
        handle_t,
        POINTER(c_int16),
        c_uint32,
    ]
    ...
    def fftBackgroundSubtractionGetRecordS16(self, backgroundRecord, size_samples):
        ...
        ats.AlazarFFTBackgroundSubtractionGetRecordS16(
            self.handle, backgroundRecord, size_samples
        )

    ats.AlazarFFTBackgroundSubtractionSetRecordS16.restype = c_uint32
    ats.AlazarFFTBackgroundSubtractionSetRecordS16.argtypes = [
        handle_t,
        POINTER(c_int16),
        c_uint32,
    ]
    ...
    def fftBackgroundSubtractionSetRecordS16(self, record, size_samples):
        """Download the record for the background subration feature to a
        board."""
        ats.AlazarFFTBackgroundSubtractionSetRecordS16(
            self.handle, record, size_samples
        )
...
class Board:
    ...
    ats.AlazarBeforeAsyncRead.argtypes = [
        handle_t,
        c_uint32,
        c_long,
        c_uint32,
    ...
    ats.AlazarConfigureSampleSkipping.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        POINTER(c_uint16),
    ...
    def configureSampleSkipping(self, mode, sampleClocksPerRecord, sampleSkipMap):
        ats.AlazarConfigureSampleSkipping(
            self.handle, mode, sampleClocksPerRecord, sampleSkipMap
        )

    ats.AlazarConfigureRecordAverage.restype = c_uint32
    ats.AlazarConfigureRecordAverage.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        c_uint32,
    ...
    def configureRecordAverage(
        self, mode, samplesPerRecord, recordsPerAverage, options
    ):
        """Co-add ADC samples into accumulator record."""
        ats.AlazarConfigureRecordAverage(
            self.handle, mode, samplesPerRecord, recordsPerAverage, options
        )
    ...
    def dspAbortCapture(self):
        """Aborts any in-progress DMA transfer, cancels any pending transfers
        and does DSP-related cleanup."""
    ...
    def dspGetNextBuffer(self, buffer, bytesToCopy, timeout_ms):
        """Equivalent of AlazarDSPGetBuffer() to call with
        ADMA_ALLOC_BUFFERS."""
    ...
    def dspGetModules(self):
        ...
        ats.AlazarDSPGetModules(
            self.handle, numModules, moduleHandlesArray, c_void_p(0)
        )
        modulesArray = []
    ...
    def getChannelInfo(self):
        """Get the on-board memory in samples per channe and sample size in
        bits per sample."""
        memorySize_samples = c_uint32(0)
        bitsPerSample = c_uint8(0)
        ats.AlazarGetChannelInfo(
            self.handle, byref(memorySize_samples), byref(bitsPerSample)
        )
    ...
    ats.AlazarGetTriggerAddress.argtypes = [
        handle_t,
        c_uint32,
        POINTER(c_uint32),
        POINTER(c_uint32),
    ...
    def getTriggerAddress(self, record):
        ...
        ats.AlazarGetTriggerAddress(
            self.handle,
            record,
            byref(address),
            byref(timestamp_high),
            byref(timestamp_low),
        )
    ...
    ats.AlazarInputControlEx.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        c_uint32,
    ...
    ats.AlazarQueryCapability.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        POINTER(c_uint32),
    ...
    def read(
        self, channelId, buffer, elementSize, record, transferOffset, transferLength
    ):
    ...
    ats.AlazarSetCaptureClock.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        c_uint32,
    ...
    def setCaptureClock(self, source, rate, edge, decimation):
        ...
        ats.AlazarSetCaptureClock(
            self.handle, int(source), int(rate), int(edge), decimation
        )
    ...
    ats.AlazarSetTriggerOperation.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        c_uint32,
    ...
    ats.AlazarSetTriggerOperationForScanning.argtypes = [
        handle_t,
        c_uint32,
        c_uint32,
        c_uint32,
    ...
    def setTriggerOperationForScanning(self, slopeId, level, options):
        """Configure the trigger engines of a board to use and external trigger
        input and, optionally, synchronize the start of an acquisition with the
        next event after the acquisition starts."""
    ...
    def triggered(self):
        """Determine if a board has triggered during the current
        acquisition."""
    ...
    ats.AlazarOCTIgnoreBadClock.argtypes = [
        handle_t,
        c_uint32,
        c_double,
        c_double,
