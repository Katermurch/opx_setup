...
with program() as PROGRAM:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, dfs)):
            update_frequency("q1_xy_ef", df + qubit_IF_q1_ef)
            play("x180", 'q1_xy')
            align()
            ...
            # qubit 2
            play("cw" * amp(saturation_scaling), "q1_xy_ef", duration=saturation_len * u.ns)
            align("q1_xy_ef", "rr1")
            measure(
                    "readout",
                    "rr1",
    ...
    with stream_processing():
        n_st.save("n")
        # resonator 1
        I_st.buffer(len(dfs)).average().save("I1")
        Q_st.buffer(len(dfs)).average().save("Q1")
        # resonator 2
...
if simulate:
    ...
    try:
        ...
        while results.is_processing():
            ...
            plt.subplot(221)
            plt.cla()
            plt.plot((dfs + qubit_IF_q1_ef) / u.MHz, R1)
            plt.ylabel(r"$R=\sqrt{I^2 + Q^2}$ [V]")
            plt.title(f"Qubit 1 - LO = {qubit_LO_q1 / u.GHz} GHz)")
            plt.subplot(223)
            plt.cla()
            plt.plot((dfs + qubit_IF_q1_ef) / u.MHz, np.unwrap(phase1))
