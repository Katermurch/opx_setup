...
with program() as resonator_spec_2D:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(f, dfs)):
            ...
            update_frequency("rr2", f + resonator_IF_q2)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.pcolor(flux, (dfs + resonator_IF_q2) / u.MHz, R)
        plt.ylabel("Readout IF [MHz]")
        plt.subplot(212)
        plt.cla()
        plt.title("Phase")
        plt.pcolor(flux, (dfs + resonator_IF_q2) / u.MHz, signal.detrend(np.unwrap(phase)))
    ...
    for i in range(len(flux)):
        minima[i] = dfs[np.argmin(R.T[i])] + resonator_IF_q2
    # Cosine fit
