...
Instead of detuning the q2_xy gates, the frame of the second x90 pulse is rotated (de-phased) to mimic an accumulated
phase acquired for a given detuning after the idle time.
This method has the advantage of playing resonant gates.

From the results, one can fit the Ramsey oscillations and precisely measure the q2_xy resonance frequency and T2*.
...
# Detuning converted into virtual Z-rotations to observe Ramsey oscillation and get the q2_xy frequency
...
with program() as ramsey:
    ...
    state = declare(bool)  # QUA variable for the q2_xy state
    I_st = declare_stream()  # Stream for the 'I' quadrature
    Q_st = declare_stream()  # Stream for the 'Q' quadrature
    state_st = declare_stream()  # Stream for the q2_xy state
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(tau, taus)):
            ...
            align()
            play('const', 'coupler_1', duration = 18)
            align()
            ...
            assign(state, I > ge_threshold_q2)
            # Save the 'I', 'Q' and 'state' to their respective streams
            save(I, I_st)
            save(Q, Q_st)
            save(state, state_st)
            # Reset the frame of the q2_xy in order not to accumulate rotations
            reset_frame("q2_xy")
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.pause(0.1)
        plt.tight_layout()
    # Fit the results to extract the q2_xy frequency and T2*
    try:
        ...
        q2_xy_T2 = np.abs(ramsey_fit["T2"][0])
        q2_xy_detuning = ramsey_fit["f"][0] * u.GHz - detuning
        plt.xlabel("Idle time [ns]")
        plt.ylabel("I quadrature [V]")
        print(f"q2_xy detuning to update in the config: q2_xy_IF += {-q2_xy_detuning:.0f} Hz")
        print(f"T2* = {q2_xy_T2:.0f} ns")
        plt.legend((f"detuning = {-q2_xy_detuning / u.kHz:.3f} kHz", f"T2* = {q2_xy_T2:.0f} ns"))
        plt.title("Ramsey measurement with virtual Z rotations")
        print(f"Detuning to add: {-q2_xy_detuning / u.kHz:.3f} kHz")
