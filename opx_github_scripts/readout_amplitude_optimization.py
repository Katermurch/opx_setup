...
if simulate:
    ...
    # Plot the data
    fig = plt.figure()
    plt.plot(amplitudes * readout_amp_q2, fidelity_vec, "b.-", label="averaged fidelity")
    plt.plot(amplitudes * readout_amp_q2, ground_fidelity_vec, "r.-", label="ground fidelity")
    plt.title("Readout amplitude optimization")
    plt.xlabel("Readout amplitude [V]")
    plt.ylabel("Readout fidelity [%]")
    plt.legend(
        (
            f"readout_amp_q2 = {readout_amp_q2 * amplitudes[np.argmax(fidelity_vec)] / u.mV:.3f} mV, for {max(fidelity_vec):.1f}% averaged fidelity",
            f"readout_amp_q2 = {readout_amp_q2 * amplitudes[np.argmax(ground_fidelity_vec)] / u.mV:.3f} mV, for {max(ground_fidelity_vec):.1f}% ground fidelity",
        )
    )
    print(
        f"The optimal readout amplitude is {readout_amp_q2 * amplitudes[np.argmax(fidelity_vec)] / u.mV:.3f} mV (Averaged fidelity={max(fidelity_vec):.1f}%)"
    )
    print(
        f"The optimal readout amplitude is {readout_amp_q2 * amplitudes[np.argmax(ground_fidelity_vec)] / u.mV:.3f} mV (Ground fidelity={max(ground_fidelity_vec):.1f}%)"
    )
    plt.show()
    # Save results
