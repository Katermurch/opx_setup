...
with program() as ac_stark_shift:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(it, iters)):  # QUA for_ loop for sweeping the number of pulses
            ...
            state, I, Q = readout_macro(threshold=ge_threshold_q2, state=state, I=I, Q=Q)
    ...
    for det in detunings:
        ...
        x180_wf, x180_der_wf = np.array(
            drag_gaussian_pulse_waveforms(
                pi_amp_q2, pi_len, pi_sigma, alpha=drag_coef_q2, anharmonicity=anharmonicity_q2, detuning=det
            )
        )
        x180_I_wf = x180_wf
        x180_Q_wf = x180_der_wf
        # Update the config
        config["waveforms"]["x180_I_wf_q2"]["samples"] = x180_I_wf.tolist()
        config["waveforms"]["x180_Q_wf_q2"]["samples"] = x180_Q_wf.tolist()
