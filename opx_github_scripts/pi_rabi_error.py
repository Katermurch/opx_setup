...
This sequence involves repeatedly executing the q2_xy pulse (such as x180, square_pi, or similar) 'N' times and
measuring the state of the rr2 across different qubit pulse amplitudes and number of pulses.
...
a_min = 0.7 #can go from -1.99 to 1.99
a_max = 1.1
n_a = 41
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.subplot(221)
        plt.cla()
        plt.pcolor(amplitudes *pi_amp_q1_ef, nb_of_pulses, I)
        plt.xlabel("Rabi pulse amplitude [V]")
        plt.ylabel("# of Rabi pulses")
        plt.title("I quadrature [V]")
        plt.subplot(222)
        plt.cla()
        plt.pcolor(amplitudes * pi_amp_q1_ef, nb_of_pulses, Q)
        plt.xlabel("Rabi pulse amplitude [V]")
        plt.title("Q quadrature [V]")
        plt.subplot(212)
        plt.cla()
        plt.plot(amplitudes * pi_amp_q1_ef, np.sum(I, axis=0))
        plt.xlabel("Rabi pulse amplitude [V]")
        plt.ylabel("Sum along the # of Rabi pulses")
        plt.pause(0.1)
        plt.tight_layout()
    print(f"Optimal x180_amp = {amplitudes[np.argmax(np.sum(I, axis=0))] * pi_amp_q1_ef:.4f} V")
    # Save results
