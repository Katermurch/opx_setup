...
n_avg = 800  # The number of averages
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"Readout frequency optimization around {resonator_IF_q2 / u.MHz} MHz")
        plt.xlabel("Readout frequency detuning [MHz]")
        plt.ylabel("SNR")
        plt.grid("on")
        plt.pause(0.1)
    plt.show()
    print(f"The optimal readout frequency is {dfs[np.argmax(SNR)] + resonator_IF_q2} Hz (SNR={max(SNR)})")
    # Save results
