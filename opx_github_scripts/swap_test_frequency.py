...
with program() as swap_test:
    ...
    f = declare(int) #drive amp pre factor
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(f, dfs)):
            update_frequency("coupler_1", f + coupler_drive_freq)

            with for_(*from_array(tau, tau_list)):

                play("x180", "q1_xy")
                align()
                play('gaussian', 'coupler_1', duration = tau)
                align()
                measure(
                    "readout",
                    "rr2",
    ...
    with stream_processing():
        I_st.buffer(len(tau_list)).buffer(len(dfs)).average().save('I')
        Q_st.buffer(len(tau_list)).buffer(len(dfs)).average().save('Q')
        n_st.save("n")
...
if simulate:
    ...
    while results.is_processing():
        ...
        #plot results 
        plt.cla()
        plt.pcolor(tau_list*4, dfs / u.MHz, I)
        plt.xlabel("time (ns)")
