...
saturation_amp = 0.8  # pre-factor to the value defined in the config - restricted to [-2; 2)
...
with program() as qubit_spec_2D:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(f, frequencies)):
            ...
            update_frequency("q2_xy", f + qubit_IF_q2)
            assign(index, 0)
            with for_(*from_array(dc, flux)):
                # Update the resonator frequency to always measure on resonance
                # update_frequency("rr2", resonator_freq[index] + resonator_IF_q2)
                # Flux sweeping
                set_dc_offset("coupler_1", "single", dc)
                wait(flux_settle_time * u.ns, "rr2", "q2_xy")
