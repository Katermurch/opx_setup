...
with program() as swap_test:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(a, amplitudes)):
            with for_(*from_array(tau, tau_list)):
                play("x180", "q1_xy")
                align()
                play('gaussian'* amp(a), 'coupler_1', duration = tau)
                align()
                measure(
                    "readout",
                    "rr2",
    ...
    with stream_processing():
        I_st.buffer(len(tau_list)).buffer(len(amplitudes)).average().save('I')
        Q_st.buffer(len(tau_list)).buffer(len(amplitudes)).average().save('Q')
        n_st.save("n")
...
if simulate:
    ...
    while results.is_processing():
        ...
        #plot results 
        plt.cla()
        plt.pcolor(tau_list, amplitudes*gauss_swap_amp, I)
        plt.xlabel("time (ns)")
        plt.ylabel("amp")
