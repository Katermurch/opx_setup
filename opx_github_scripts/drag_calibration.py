...
from macros_jr import readout_macro
        ...
        with for_(*from_array(a, amps)):  # QUA for_ loop for sweeping the pulse amplitude
            with for_(*from_array(it, iters)):  # QUA for_ loop for sweeping the number of pulses
                ...
                with for_(pulses, iter_min, pulses <= it, pulses + d):
                    ...
                    play("x180" * amp(-1, 0, 0, -a), "q1_xy")
                # Align the two elements to measure after playing the q2_xy pulses.
                align()
                # play('const', 'coupler_1', duration = 18)
                # align()
                # Measure the rr2 and extract the qubit state
                state, I, Q = readout_macro(threshold=ge_threshold_q2, state=state, I=I, Q=Q, element="rr1")
